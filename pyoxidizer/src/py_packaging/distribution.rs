...
pub fn resolve_python_paths(base: &Path, python_version: &str) -> PythonPaths {
    ...
    let dll_dir = if stdlib == unix_lib_dir {
	    p.join("lib")
    }
    else {
        prefix.clone()
    ...
    pub fn create_hacked_base(&self, logger: &slog::Logger) -> PythonPaths {
        ...
        for (_, path) in self.libraries.iter() {
            ...
            if !target.exists() {
                ...
                if target.display().to_string().contains("_static") {
                    let target = python_paths.dll_dir.join(path.file_name().unwrap().to_str().unwrap().replace("_static", ""));
				    symlink_file(&path, &target).unwrap();
                    warn!(logger, "added {}", target.display().to_string());
                }
            }
